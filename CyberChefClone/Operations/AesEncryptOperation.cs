...
public class AesEncryptOperation : IOperation
{
    public string Name => "AES Encrypt";
    public string Description => "Encrypt data using AES algorithm";
    ...
    public Dictionary<string, ParameterInfo> ParameterTypes => new()
    {
        ...
        ["InputFormat"] = new ParameterInfo
        {
            ...
            Description = "Input data format: Hex, Base64 or string (UTF-8)"
        },
        ...
        ["OutputFormat"] = new ParameterInfo
        {
            ...
            DefaultValue = "Hex",
            Options = new List<string> { "Hex", "Base64", "String" },
            Description = "Encrypted data output format"
        }
    };

    public Task<byte[]> ExecuteAsync(byte[] input)
    {
        Console.WriteLine($"=== AES Encrypt ExecuteAsync ===");
        ...
        Console.WriteLine($"Mode: '{mode}', Padding: '{padding}', OutputFormat: '{outputFormat}'");
        ...
        switch (inputFormat)
        {
            case "Hex":
                ...
                dataToEncrypt = new byte[hex.Length / 2];
                for (int i = 0; i < hex.Length; i += 2)
                {
                    dataToEncrypt[i / 2] = Convert.ToByte(hex.Substring(i, 2), 16);
                }
                Console.WriteLine($"InputFormat Hex: {dataToEncrypt.Length} bytes");
                break;
            case "Base64":
                dataToEncrypt = Convert.FromBase64String(inputString);
                Console.WriteLine($"InputFormat Base64: {dataToEncrypt.Length} bytes");
                break;
            default: // String (UTF-8)
                dataToEncrypt = Encoding.UTF8.GetBytes(inputString);
                Console.WriteLine($"InputFormat String: {dataToEncrypt.Length} bytes");
                break;
        }
        ...
        switch (outputFormat)
        {
            ...
            case "String":
                ...
                {
                    result = Encoding.UTF8.GetBytes(Convert.ToBase64String(encrypted));
                }
                break;
            default: // Base64
